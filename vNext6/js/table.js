const TableOps = {
    ...
    moveColumn(tableName, colName, direction) {
        ...
        if (SqlUtils.isComplexSchema(createSQL)) {
            UI.toast("此資料表包含複雜 schema，為避免資料結構損壞，目前不支援欄位排序。建議做法：1. 先匯出 SQL 2. 手動調整 CREATE TABLE 結構 3. 建立新表並搬移資料。", "error");
            return;
        }
        ...
        if (triggers.length > 0 && triggers[0].values.length > 0) {
            UI.toast("此資料表有 trigger，為避免資料結構損壞，目前不支援欄位排序。", "error");
            return;
        }
        ...
        const msg = `【實驗功能】欄位排序會重建資料表 "${tableName}"。\n\n此功能仍屬實驗功能，雖然系統會阻擋部分複雜 schema，但仍建議先另存資料庫備份。\n\n是否繼續？`;
        if (!this.confirmDanger(msg)) return;
        ...
        document.getElementById("importCSVConfirm").onclick = () => {
            ...
            reader.onload = (e) => {
                try {
                    ...
                    // Clean up column names - empty names become column_N
