const UI = {
      ...
      if (save) {
        ...
        if (newVal !== (currentVal === null ? '' : String(currentVal))) {
          try {
            ...
            DB.run(`UPDATE ${SqlUtils.quoteIdent(tableName)} SET ${SqlUtils.quoteIdent(col)} = ? WHERE ${whereParts.join(' AND ')}`, [newVal === '' ? null : newVal, ...whereValues]);
            this.loadDataPage(tabId);
  ...
  openStructureTab(tableName) {
    ...
    structHTML += '<th style="width:80px">操作 <span style="font-size:9px;color:var(--yellow);font-weight:normal">實驗</span></th><th>欄位</th><th>類型</th><th>允許空值</th><th>預設值</th><th>主鍵</th></tr></thead><tbody>';
      ...
      structHTML += `<tr>
        <td>
          <div style="display:flex;gap:2px">
            <button class="icon-btn" data-action="move-up" data-col="${this.esc(col.name)}" ${idx === 0 ? 'disabled' : ''} style="width:24px;height:24px" title="上移">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="18,15 12,9 6,15"/></svg>
            </button>
            <button class="icon-btn" data-action="move-down" data-col="${this.esc(col.name)}" ${idx === info.length - 1 ? 'disabled' : ''} style="width:24px;height:24px" title="下移">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6,9 12,15 18,9"/></svg>
            </button>
            <button class="icon-btn" data-action="delete-col" data-col="${this.esc(col.name)}" style="width:24px;height:24px" title="刪除欄位">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--red)" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
            </button>
          </div>
        </td>
    ...
    panel.querySelectorAll('[data-action="delete-col"]').forEach(btn => {
      btn.onclick = () => TableOps.deleteColumn(tableName, btn.dataset.col);
    });
  },
  ...
  openForeignKeysTab(tableName) {
    ...
    try {
      const result = DB.query(`PRAGMA foreign_key_list(${SqlUtils.quoteIdent(tableName)})`);
      ...
      try {
        ...
        allTables.forEach(t => {
          ...
          const res = DB.query(`PRAGMA foreign_key_list(${SqlUtils.quoteIdent(t)})`);
          if (res.length > 0) {
  ...
  openIndexesTab(tableName) {
    ...
    try {
      const result = DB.query(`PRAGMA index_list(${SqlUtils.quoteIdent(tableName)})`);
      ...
      if (indexes.length === 0) {
        ...
        indexes.forEach(idx => {
          ...
          let cols = [];
          try {
            const colResult = DB.query(`PRAGMA index_info(${SqlUtils.quoteIdent(name)})`);
