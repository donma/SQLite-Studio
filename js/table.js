const TableOps = {
      ...
      <button class="btn btn-primary" id="createTableConfirm">建立表</button>`;
        ...
        document.getElementById("createTableConfirm").onclick = () => {
            ...
            rows.forEach((row) => {
                ...
                if (!name) return;
                let col = `${SqlUtils.quoteIdent(name)} ${type}`;
            ...
            let sql = `CREATE TABLE ${t} (${cols.join(", ")}`;
            if (pkCols.length > 0 && !rows[0].querySelector(".col-pk").checked) {
                sql += `, PRIMARY KEY (${pkCols.map((c) => SqlUtils.quoteIdent(c)).join(", ")})`;
            }
            sql += ")";

            if (pkCols.length === 1) {
                const pkRow = Array.from(rows).find((r) =>
                    r.querySelector(".col-pk").checked
                );
                if (pkRow && pkRow.querySelector(".col-type").value === "INTEGER") {
                    sql = `CREATE TABLE ${t} (${cols.join(", ")}, PRIMARY KEY(${SqlUtils.quoteIdent(pkCols[0])} AUTOINCREMENT))`;
                }
            }
    ...
    moveColumn(tableName, colName, direction) {
        ...
        try {
            ...
            const insertSQL = `INSERT INTO ${SqlUtils.quoteIdent(tmpName)} (${colList}) VALUES (${placeholders})`;
            ...
            const relatedSQL =
                masterSQL.length > 0 ? masterSQL[0].values : [];

            // Drop original and rename
            DB.execute(`DROP TABLE ${t}`);
            DB.execute(
                `ALTER TABLE ${SqlUtils.quoteIdent(tmpName)} RENAME TO ${t}`
            );
